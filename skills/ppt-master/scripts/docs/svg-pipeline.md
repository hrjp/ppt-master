...
python3 scripts/svg_to_pptx.py <project_path> --merge-paragraphs
...
- Paragraph merging is disabled by default and prioritizes PowerPoint line fidelity:
  - Default: every dy-stacked `<tspan>` becomes its own text frame, preserving the SVG's authored line breaks and reducing post-export rewrap surprises
  - With `--merge-paragraphs`: mergeable paragraph blocks (same x, dy clustered around one base line-height, optional larger gap for paragraph breaks) collapse into one editable text frame with multiple `<a:p>`
  - Side effect: merged paragraphs are easier to edit as one box, but PowerPoint may wrap them to a different line count than the SVG source. Use `--merge-paragraphs` only when that editability is worth the fidelity trade-off
  - Mergeable detection is conservative: only clean paragraph blocks merge; mixed-layout `<text>` still falls back to per-line text frames
