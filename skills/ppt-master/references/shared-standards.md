...
- **Hard rule — text stays inside its box**: every text block must be wrapped and laid out so the final rendered text stays inside the page canvas and inside its intended visual container. Text clipping past a card edge, footer band, label box, or page boundary is a failure. For Japanese / CJK text, prefer shorter lines and one extra wrap over dense lines that are likely to overflow after PPT export.
- **Hard rule — do not edge-fit text for PPT**: a block that only barely fits in SVG is unsafe for PPT export. Inside any card / panel / label box, leave visible slack on the right and bottom edges instead of packing the last line tight to the border. As a practical default, keep at least roughly half a body line of spare depth below multi-line text and avoid same-column text blocks whose vertical gap is only a few pixels; PPT line metrics may grow enough to make them collide.
- **Fonts**: every `font-family` stack MUST end with a pre-installed family (`Yu Gothic` / `Meiryo` / Microsoft YaHei / SimSun / Arial / Times New Roman / Consolas …); `@font-face` is FORBIDDEN. **Default — Japanese PPTs use `Yu Gothic`** as the CJK lead unless the user or template explicitly chooses another family. Full rule: [`strategist.md §g`](strategist.md).
