...
[English](./README_EN.md) | [中文](./README_CN.md) | [日本語](./README.md)

> [!IMPORTANT]
> ### Recent update: better Japanese/CJK output stability
> The latest commit added stricter Japanese/CJK text handling to reduce post-export layout breakage in PowerPoint.
> - Conservative Japanese/CJK line wrapping to avoid boxes that technically fit in SVG but overflow after PPTX export
> - Default Japanese font fallback to `Yu Gothic` with `Meiryo` fallback when a deck is underspecified
> - New `svg_quality_checker.py` overflow / overlap checks to catch clipped text, edge-hugging text boxes, and likely export-time rewrap issues before PPTX export
